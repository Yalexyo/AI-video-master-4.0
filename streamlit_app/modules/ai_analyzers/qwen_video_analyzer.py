...
"""
    ...
    def _get_fallback_retry_prompt(self) -> str:
        ...
        try:
            ...
            return f"""
# **身份**
你是一位顶级的母婴行业视觉分析专家，拥有火眼金睛，能够从视频帧中精准识别营销要素。
...
2.  **品牌铁律**: `brand_elements`字段**只能**从"品牌词库"中选择。如果画面中没有明确出现这些品牌，该字段必须为"无"。**绝对禁止**识别任何其他品牌。
3.  **关键物体**: 对 "奶粉罐"、"奶瓶" 这类关键物体要进行最优先识别。
4.  **拒绝模糊**: 不要使用"疑似"、"可能"等任何不确定的描述。如果无法确定，请将该字段留空或标记为"无"。
5.  **结构化**: 严格按照下面的"输出格式"返回结果，不要有任何多余的解释。
...
"""
    ...
    def _get_fallback_audio_prompt(self) -> str:
        ...
        try:
            ...
            prompt = f"""
你是一个专业的母婴行业内容分析师。请根据以下录音文本，提取内容标签。
...
"""
            ...
            if result['brand_elements']:
                ...
                for brand in detected_brands_list:
                    ...
                    if brand.lower() in allowed_brands:
                        ...
                        logger.warning(f"🚫 [品牌过滤] 已移除不在配置中的品牌: '{brand}'")
    ...
    def _get_targeted_analysis_prompt(self, transcription: str, visual_result: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # 关键优化：在混合分析Prompt中同样明确品牌范围
            return f"""
# **身份**
你是一位结合多模态信息（视觉+语音）的母婴领域高级分析专家。
...
- **品牌铁律**: 品牌(brand_elements)的识别必须遵守铁律：**只能**从"{core_brands_text}"中选择。如果在语音或视觉中没有明确提到这些品牌，该字段必须为"无"。**绝对禁止**识别任何其他品牌。
...
# **输出格式** (严格遵守，只需输出补充信息)
- **Object_Supplement**: [语音中提到的、可补充的物体信息]

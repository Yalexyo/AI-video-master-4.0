...
class QwenVideoAnalyzer:
    ...
    def _build_analysis_prompt(self, tag_language: str) -> str:
        ...
        # confidence 为单个浮点数值
        return """请分析视频内容，并按以下格式输出简洁结构化结果：

**重要：请保持标签简洁，避免冗长描述**

object: [物体名词，去掉修饰语。例如：婴儿,奶瓶,玩具,汽车]
sence: [核心场景，只保留主要地点/环境。例如：卧室,客厅,公园,医院]
emotion: [基础情绪词。例如：开心,难过,兴奋,平静]
brand_elements: [品牌相关物品名词。例如：奶粉罐,Logo,包装盒,说明书]
confidence: [置信度数值，0.0到1.0]
...
sence: 卧室
emotion: 开心
brand_elements: 奶粉罐,Logo
confidence: 0.85

错误示例（避免）：
object: 可爱的小婴儿,透明的塑料奶瓶,五颜六色的毛绒玩具
sence: 温馨的家庭卧室环境,室内光线充足的房间
emotion: 非常开心愉悦的表情
"""
